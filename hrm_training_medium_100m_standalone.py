...
VERSIÓN MEDIUM: Configuración para ~100M parámetros con contexto extendido (1024 tokens) [STANDALONE VERSION]
...
class AdaptiveBPETokenizer:
    ...
    def __init__(self, vocab_size=16000, min_frequency=2):
...
OUTPUT_DIR = os.path.join(OUTPUT_BASE, "HRM-Medium-100M-standalone-v1.0-output")
...
if not os.environ.get('HRM_IMPORT_ONLY'):
    ...
    tokenizer = AdaptiveBPETokenizer(vocab_size=16000)
...
patience_counter = 0
CHECKPOINT_STEPS = 750  # Checkpoints moderados para modelo medium (100m)
global_step = 0
