...
class HRMText1Config(ConfigBase):
    ...
    def __init__(self,
                 ...
                 n_embd=384,                # Small model embeddings - escalado apropiadamente
                 n_head=12,                 # Small model heads - múltiplo de n_embd
                 n_layers=6,                # Small model layers - balance entre capacidad y estabilidad
                 d_ff=1536,                 # Small model FFN - 4x n_embd
...
class HRMInner(nn.Module):
    ...
    def forward(self, z_H, z_L, input_embeddings, attn_mask=None, key_padding_mask=None, training=True):
        ...
        # HRM Cycles implementation for LLMs (small model: moderate cycles)
...
def train_hrm_distributed(
    tokenizer_name: str = "openai-community/gpt2",
    output_dir: str = "./hrm-small-50m-distributed",
    num_train_samples: int = 100000,  # Más samples para aprovechar múltiples GPUs
    num_val_samples: int = 10000,     
    batch_size: int = 8,             # Batch size por GPU
    ...
    if is_main_process:
        print(f"🚀 Iniciando entrenamiento HRM Small 50M DISTRIBUIDO")
    ...
    # Crear configuración del modelo Small 50M
    ...
    if is_main_process:
        print(f"🧠 Modelo Small 50M creado:")
    ...
    if is_main_process:
        print(f"🎯 Entrenamiento distribuido configurado:")
    ...
    if is_main_process:
        print(f"\n🎉 ¡Iniciando entrenamiento distribuido Small 50M!")
        print("=" * 60)
    ...
    try:
                ...
                if step % eval_steps == 0 and is_main_process:
                    ...
                    with torch.no_grad():
                        for val_batch in val_loader:
                            if val_batches >= 20:  # Evaluar solo 20 batches
                                break
        ...
        if is_main_process:
            ...
            print(f"\n✅ ¡Entrenamiento distribuido Small 50M completado!")
...
def main():
    parser = argparse.ArgumentParser(description="Entrenar HRM Small 50M con entrenamiento distribuido")
    parser.add_argument("--tokenizer", type=str, default="openai-community/gpt2",
                       help="Nombre del tokenizador HF")
    parser.add_argument("--output_dir", type=str, default="./hrm-small-50m-distributed",
    ...
    if len(os.sys.argv) == 1:
        ...
        print("\nUso con torchrun:")
        print("  torchrun --nproc_per_node=2 hrm_training_small_50m_distributed.py")
        print("  torchrun --nproc_per_node=4 hrm_training_small_50m_distributed.py")
        print("  torchrun --nproc_per_node=8 hrm_training_small_50m_distributed.py")
        print("\nEjemplos:")
        print("  # 2 GPUs")
        print("  torchrun --nproc_per_node=2 hrm_training_small_50m_distributed.py --batch_size 8")
        print("  # 4 GPUs")
        print("  torchrun --nproc_per_node=4 hrm_training_small_50m_distributed.py --batch_size 6")
        print("  # 8 GPUs")
        print("  torchrun --nproc_per_node=8 hrm_training_small_50m_distributed.py --batch_size 4")
        return
