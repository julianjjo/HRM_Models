# -*- coding: utf-8 -*-
"""
HRM-Models Training Script con Tokenizador HuggingFace - MODELO LARGE ~1B PARÁMETROS
...
- Arquitectura HRM optimizada para 1B parámetros
...
class HRMText1Config(ConfigBase):
    ...
    def __init__(self,
                 ...
                 block_size=1024,           # Large model context - incrementado considerablemente
                 n_embd=1536,              # Large model embeddings - escalado apropiadamente
                 n_head=24,                # Large model heads - múltiplo de n_embd
                 n_layers=24,              # Large model layers - balance entre capacidad y estabilidad
                 d_ff=6144,               # Large model FFN - 4x n_embd
...
class HRMInner(nn.Module):
    ...
    def forward(self, z_H, z_L, input_embeddings, attn_mask=None, key_padding_mask=None, training=True):
        ...
        # HRM Cycles implementation for LLMs (large model: full cycles)
...
def train_hrm_hf(
    ...
    output_dir: str = "./hrm-large-1b-hf",
    num_train_samples: int = 1000000,  # Incrementado para modelo más grande
    num_val_samples: int = 100000,     # Incrementado proporcionalmente
    batch_size: int = 1,             # Reducido para acomodar modelo más grande
    learning_rate: float = 3e-6,     # Reducido para mejor estabilidad
    num_epochs: int = 3,
    save_steps: int = 300,
    eval_steps: int = 75,            # Evaluación más frecuente
    max_grad_norm: float = 0.5,     # Menos agresivo que micro
    warmup_steps: int = 5000,       # Incrementado para modelo más grande
    ...
    """Entrenar modelo HRM Large 1B con tokenizador HuggingFace"""
    ...
    print(f"🚀 Iniciando entrenamiento HRM Large 1B con tokenizador HF")
    print(f"📊 Configuración:")
    ...
    # Crear configuración del modelo Large 1B
    ...
    print(f"📐 Configuración del modelo Large 1B:")
    ...
    print(f"🧠 Modelo Large 1B creado:")
    ...
    # Para entrenamientos grandes (1B+), usar warmup más largo
    if len(train_texts) >= 10000:  # Si >= 10K samples
        effective_warmup_steps = max(warmup_steps, total_steps // 15)  # ~6.7% warmup
        print(f"🔥 Modelo Large 1B: Warmup extendido a {effective_warmup_steps} steps")
    else:
    ...
    print(f"🎯 Entrenamiento Large 1B configurado:")
    ...
    print(f"\n🎉 ¡Iniciando entrenamiento Large 1B!")
    print("=" * 60)
    ...
    print(f"\n✅ ¡Entrenamiento Large 1B completado!")
...
def main():
    parser = argparse.ArgumentParser(description="Entrenar HRM Large 1B con tokenizador HuggingFace optimizado")
    parser.add_argument("--tokenizer", type=str, default="openai-community/gpt2",
                       help="Nombre del tokenizador HF")
    parser.add_argument("--output_dir", type=str, default="./hrm-large-1b-hf",
    ...
    if len(os.sys.argv) == 1:
        print("🚀 HRM Large 1B Training con Tokenizador HuggingFace")
        print("\nUso:")
        print("  python hrm_training_large_1b_standalone_hf.py [opciones]")
        print("\nEjemplos:")
        print("  # Entrenar con GPT2 inglés (configuración automática)")
        print("  python hrm_training_large_1b_standalone_hf.py --tokenizer openai-community/gpt2")
        print("  ")
        print("  # NO STREAMING (descarga completa, recomendado para entrenamientos grandes)")
        print("  python hrm_training_large_1b_standalone_hf.py --no_streaming --train_samples 50000")
        print("  ")
        print("  # Modo CPU INTENSIVO")
        print("  python hrm_training_large_1b_standalone_hf.py --cpu_intensive --batch_size_multiplier 2")
        print("  ")
        print("  # Configuración manual de workers + no streaming")
        print("  python hrm_training_large_1b_standalone_hf.py --num_workers 8 --no_streaming")
        print("  ")
        print("  # Dataset diferente en español")
        print("  python hrm_training_large_1b_standalone_hf.py --tokenizer DeepESP/gpt2-spanish --no_streaming")
        return
