...
VERSIÓN NANO: Configuración para ~25M parámetros con contexto optimizado (512 tokens) [STANDALONE VERSION]
...
HF_REPO_ID = f"dreamwar/HRM-Models-Nano-25M"
SEED = 42
...
OUTPUT_DIR = os.path.join(OUTPUT_BASE, "hrm_models_nano_25m_output")
    ...
    if HF_TOKEN:
        try:
            ...
            model_to_save.push_to_hub(
                HF_REPO_ID,
                token=HF_TOKEN,
                commit_message=f"Upload HRM-Models 25M nano model"
            )
            ...
            tokenizer.push_to_hub(
                HF_REPO_ID,
                token=HF_TOKEN,
                commit_message=f"Upload tokenizer for HRM-Models 25M nano model"
            )
